...
import { computeRotatedAABB } from '@/engine/geometry/aabb';
import type { AABB, QuadItem } from '@/engine/types';

export interface StressTestItem extends QuadItem {
  id: number;
  dirty: boolean;
  tx: number;
  ty: number;
...
export class PidStressTester {
  ...
  generate(count: number) {
    ...
    console.log(`✅ PidStressTester: 生成 ${count} 个模拟图元`);
  }
